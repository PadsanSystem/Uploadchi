...
if (!iADMIN || !defined("iAUTH") || !isset($aid) || $aid != iAUTH) { redirect(BASEDIR.'index.php'); }

if(isset($id) && isset($action) && ($action=='delete')){
	$database->delete('users', ['user_id'=>$id]);
}else if(isset($id) && isset($action) && ($action=='Enable')){
	$database->update(DB_PREFIX.'users', ['user_status'=>'Disable'], ['user_id'=>$id]);
}else if(isset($id) && isset($action) && ($action=='Disable')){
	$database->update(DB_PREFIX.'users', ['user_status'=>'Enable'], ['user_id'=>$id]);
}
?>
<div id="page-wrapper">
	<div class="row">
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<table class="table table-striped table-bordered table-hover" id="tables">
						...
						<tbody>
						<?php
						// $database-select(DB_PREFIX.'users', );
